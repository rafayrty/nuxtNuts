  ...
  <div class="start">
    ...
    <h1 class="text-lg flex items-center">
      <span class="hidden lg:block">
        {{products.total}} {{$t('sorting.results')}}
      </span>

      <span class="lg:hidden">
        {{products.total}} {{$t('sorting.results_mob')}}
      </span>
            ...
            <div class="product" v-for="product in products" :key="product.id">
              ...
              <div class="product-image">
                <nuxt-link :to="localePath(`/product/${product.id}`)">
                  <img :src="product.image" v-shared-element:[illusoryId(product.id)] alt="">
                  <span class="gm" v-if="product.kind==0"> 100 {{$t('gm')}}</span>
                </nuxt-link>
              ...
              <div class="product-text">
                ...
                <div class="product-title">
                  <h4>{{product.descriptions[0].name}}</h4>
                </div>
                <div class="actions">
                  <div class="price">
                    <h4>₪ {{product.price}}</h4>
...
export default {
     async asyncData(context) {
       ...
       try {
         const products = await context.app.$axios.$get(`/api/product/search/${type}/${context.app.i18n.locale}/${search}`)
         return {
           products
         }
       } catch (e) {
   ...
   async $route(to, from) {
     ...
     created(){
       this.pagination.last_page = this.products.last_page;
     },
     methods:{
            illusoryId(id){
      return "product_"+id;
    ...
    paginate(index){
      //Passing Page index through ?page=1
        ...
        this.$router.push(this.localePath({path:`/search?search=${this.$route.query.search}&type=${this.type}`}));
