...
<div class="address mt-4 lg:flex justify-between items-center">
    <h1 class="text-2xl font-bold">{{$t('account.my_address')}}</h1>
...
<div key="0" class="addresses-list mt-8" v-if="addresses.length">
      <transition-group class="relative" name="list"  tag="ul">

<li class="address-list fruit-list-item bg-white shadow-lg" v-for="(address,index) in addresses"  :key="address.id">
<div class="list-header bg-gray-100 text-blue-500 font-semibold py-1 px-4 lg:px-8 font-xs">
    Address {{index+1}}
</div>
<div class="full-address flex justify-between items-center py-5 px-4 lg:px-8">
    <p>
    ...
    <div class="actions flex items-center">
        <div class="edit">
<nuxt-link :to="localePath('/account/address/edit/'+address.id)">
...
<div key="1" class="empty-state mt-8" v-else>
